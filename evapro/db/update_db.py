...
PERSONAl_DB = str(Path.home() / '.annoeva' / 'projects.db')  # 数据库路径常量
...
def get_analysis_project(connection, create_date):
    ...
    try:
        # 使用参数化查询
        df = pd.read_sql(query, con=connection, params=(create_date))
...
def lims2evaproDB() -> None:
    ...
    one_week_ago = now - timedelta(weeks=1)  # 或者 days=7
    ...
    path_df = path_df[path_df['SUB_PROJECT_ID'].isin(df_ana_pro[' v'])]
    ...
    for i, row in df_ana_pro.iterrows():
        try:
            ...
            tbj.insert_allpro_tb_sql(row['info_user_id'], row['info_user_id'], row['PRODUCT'], isautoflow, row['workdir'])
        except Exception as e:
...
def add_project2annoeva() -> None:
    ...
    query = """
        ...
        WHERE 
            ...
            AND workdir != ''
    """
    df = read_sql(query, con=pro_tbj.conn)
    for i, row in df.iterrows():
        try:
            p = subprocess.Popen(f'{conf['annoeva']} addproject -p {row['proid']} -t {row['ptype']} -d {row['workdir']}', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            #sys.stdout.write('e:'+str(stde,'utf-8')+'\n')
            continue
